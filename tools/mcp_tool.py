...
_OAUTH_INITIALIZE_GRACE = 5      # let the callback timeout unwind cleanly
...
def _mcp_initialize_timeout(
    config: dict,
    *,
    server_name: str,
    auth_type: str | None,
) -> float:
    """Return the timeout for the MCP initialize handshake.

    A dashboard OAuth retry performs the browser authorization from inside
    ``session.initialize()``.  The normal transport timeout is shorter than
    the advertised browser callback window, so applying it unchanged cancels
    a still-valid consent flow.  Extend only an active dashboard flow; cached
    OAuth sessions and ordinary transports retain the normal fail-fast bound.
    """
    ...
    callback_timeout = float(oauth_config.get("timeout", 300))
    return max(connect_timeout, callback_timeout + _OAUTH_INITIALIZE_GRACE)
...
def retry_mcp_server(server_name: str) -> dict:
    """Retry exactly one configured MCP transport and return its new status.

    Unlike ``discover_mcp_tools``/``shutdown_mcp_servers``, this preserves every
    unrelated MCP session. A parked live task is signalled to rebuild its own
    transport; a server that failed before registration is discovered on its
    own. Manual retry also clears that server's circuit-breaker and rate-limit
    bookkeeping so the request is acted on immediately.
    """
    ...
    if not _parse_boolish(config.get("enabled", True), default=True):
        return next(
            item for item in get_mcp_status() if item.get("name") == server_name
        )
    ...
    if server is None:
        ...
        if not _signal_reconnect(server):
            raise RuntimeError(
                f"MCP server '{server_name}' has no reconnect mechanism"
            )
        try:
            timeout = float(config.get("connect_timeout", 15) or 15)
        except (TypeError, ValueError):
            timeout = 15.0
